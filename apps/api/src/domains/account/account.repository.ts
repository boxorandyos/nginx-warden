...
export class AccountRepository {
  ...
  /**
   * Revoke a session by session ID (matches refresh JWT jti).
   * Revokes refresh tokens with that jti and deletes the UserSession row.
   */
  async revokeSession(userId: string, sessionId: string): Promise<boolean> {
    return prisma.$transaction(async (tx) => {
      await tx.refreshToken.updateMany({
        where: { userId, jti: sessionId, revokedAt: null },
        data: { revokedAt: new Date() },
      });

      const res = await tx.userSession.deleteMany({
