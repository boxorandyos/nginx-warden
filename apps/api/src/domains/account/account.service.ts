...
export class AccountService {
   ...
   * Get active sessions
   */
  async getSessions(userId: string, currentSessionId?: string): Promise<SessionListItem[]> {
    const rows = await this.accountRepository.getActiveSessions(userId);
    return rows.map((s) => ({
  ...
  async revokeSession(userId: string, sessionId: string): Promise<void> {
    const deleted = await this.accountRepository.revokeSession(userId, sessionId);
    if (!deleted) {
