...
export function CustomRuleDialog({ open, onOpenChange, editRule }: CustomRuleDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !category.trim() || !ruleContent.trim()) {
      toast.error(t('modsec.customRule.missingFields'));
      return;
    }
    ...
    try {
      if (isEditMode && editRule) {
        ...
        toast.success(t('modsec.customRule.updated'));
        ...
        toast.success(t('modsec.customRule.added'));
        // Only close dialog on success
        onOpenChange(false);
      }
    } catch (error: any) {
      const errorMessage = error?.response?.data?.message || t('modsec.customRule.saveFailed');
