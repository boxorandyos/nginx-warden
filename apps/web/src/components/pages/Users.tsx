...
function UserStatsCards() {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {t('users.stats.activeHint', { count: stats.data.active })}
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t('users.stats.recentLogins')}</CardTitle>
          <UserPlus className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{stats.data.recentLogins}</div>
          <p className="text-xs text-muted-foreground">
            {t('users.stats.recentLoginsHint')}
          </p>
        </CardContent>
      </Card>
    </div>
...
function UsersTable() {
  ...
  const handleAddUser = async () => {
    ...
    if (!formData.username || !formData.email || !formData.fullName) {
      toast({
        ...
        description: t("users.validation.fields"),
        variant: "destructive"
      });
      return;
    }
    ...
    if (!editingUser && !formData.password) {
      toast({
        title: t("common.validationError"),
        description: t("users.validation.password"),
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      if (editingUser) {
        ...
        toast({ title: t("users.toast.updateSuccess") });
      } else {
        // Create new user
        ...
        toast({
          title: t("users.toast.createSuccess"),
          description: t("users.toast.invited")
        });
      }
      ...
      toast({
        title: editingUser ? t("users.error.update") : t("users.error.create"),
        description: error.response?.data?.message || t("users.error.generic"),
        variant: "destructive"
      });
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      toast({ title: t("users.toast.deleteSuccess") });
    } catch (error: any) {
      toast({
        title: t("users.error.deleteTitle"),
        description: error.response?.data?.message || t("users.toast.deleteFailed"),
        variant: "destructive"
      });
    }
  };
  ...
  const handleToggleStatus = async (id: string) => {
    ...
    try {
      ...
      toast({ title: t("users.toast.statusSuccess", { status: newStatus === 'active' ? t("users.status.active") : t("users.status.inactive") }) });
    } catch (error: any) {
      toast({
        title: t("users.error.statusTitle"),
        description: error.response?.data?.message || t("users.toast.statusFailed"),
        variant: "destructive"
      });
    }
  };
  ...
  const confirmResetPassword = async () => {
    ...
    try {
      ...
      toast({
        title: t("users.error.resetPassword"),
        description: error.response?.data?.message || error.message || t("users.toast.resetFailed"),
        variant: "destructive"
      });
  ...
  const handleCopyPassword = async () => {
    if (!resetPasswordDialog.newPassword) {
      toast({
        title: t("users.noPasswordCopy"),
        description: t("users.passwordEmpty"),
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (copySuccess) {
        ...
        toast({ 
          title: t("users.copySuccessTitle"),
          description: t("users.copySuccessDesc", { method: copyMethod }),
          duration: 5000
        });
      } else {
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('users.table.title', { count: users.data.length })}</CardTitle>
          <CardDescription>{t('users.table.desc')}</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('users.roleSection.title')}</CardTitle>
          <CardDescription>{t('users.roleSection.desc')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              {t('users.role.adminDesc')}
            </p>
          </div>
          <div className="flex items-start gap-3">
            <Badge variant="default">
              <Shield className="h-3 w-3 mr-1" />
              {t('users.role.moderator')}
            </Badge>
            <p className="text-sm text-muted-foreground">
              {t('users.role.moderatorDesc')}
            </p>
          </div>
          <div className="flex items-start gap-3">
