...
const LogEntries = ({
  ...
  const handleDownloadLogs = useCallback(async () => {
    try {
      ...
      toast({
        title: t("logs.downloadSuccessTitle"),
        description: t("logs.downloadSuccessDesc"),
      });
    } catch (error: any) {
      console.error("Failed to download logs:", error);
      toast({
        title: t("common.error"),
        description: error.response?.data?.message || t("logs.downloadFailed"),
        variant: "destructive",
      });
    }
  }, [level, type, domain, search, ruleId, uniqueId, toast, t]);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("logs.cardTitle", { count: pagination.total })}</CardTitle>
        <CardDescription>{t("logs.cardSubtitle")}</CardDescription>
      </CardHeader>
      <CardContent>
        {/* Filters */}
        <div className="flex flex-col gap-3 mb-6 lg:flex-row lg:items-center lg:flex-wrap">
          ...
          <FilterInput
            placeholder={t("logs.uniqueIdPlaceholder")}
            value={uniqueId || ""}
          ...
          <Select value={domain} onValueChange={(val) => dispatchCustomEvent('domainChange', val)}>
            <SelectTrigger className="w-full lg:w-[160px]">
              <SelectValue placeholder={t("logs.filterDomain")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("logs.allDomains")}</SelectItem>
              {domains.map((d) => (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline">
                {t("logs.columns")} <ChevronDown className="ml-2 h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
        ...
        <div className="flex items-center justify-between space-x-2 py-4">
          ...
          <div className="flex items-center space-x-2">
            <div className="flex items-center space-x-2">
              <p className="text-sm font-medium">{t("logs.rowsPerPage")}</p>
              <Select
                value={`${limit}`}
                onValueChange={(value) => {
            ...
            <div className="flex w-[100px] items-center justify-center text-sm font-medium">
              {t("logs.pageOf", { page, total: pagination.totalPages || 1 })}
            </div>
            <div className="flex items-center space-x-2">
              {[
...
const Logs = () => {
  ...
  const handleDownloadLogs = useCallback(async () => {
    try {
      ...
      await downloadLogs(downloadParams);
      toast({ title: t("logs.downloadSuccessTitle"), description: t("logs.downloadSuccessDesc") });
    } catch (error: any) {
      console.error("Failed to download logs:", error);
      toast({
        title: t("common.error"),
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{t("logs.pageTitle")}</h1>
            <p className="text-muted-foreground">{t("logs.pageSubtitle")}</p>
          </div>
        </div>
