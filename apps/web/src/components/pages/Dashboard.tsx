  ...
  const statsCards = [
    {
      ...
      description: t("dashboard.stats.domainsDesc", {
          ...
          <CardContent>
            <p className="text-sm">
              {t("dashboard.activeAlerts.body", {
                ...
                criticalPart:
                  criticalAlerts > 0
                    ? t("dashboard.activeAlerts.criticalPart", {
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>{t("dashboard.cpu.title")}</CardTitle>
              <CardDescription>{t("dashboard.chart.last24h")}</CardDescription>
            </div>
            {stats?.system && (
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  {t("dashboard.cpu.cores", { count: stats.system.cpuCores })}
                </div>
              </div>
            )}
          </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>{t("dashboard.memory.title")}</CardTitle>
              <CardDescription>{t("dashboard.chart.last24h")}</CardDescription>
            </div>
            {stats?.system && (
              <div className="text-right">
        ...
        <CardContent>
          {metrics?.memory && metrics.memory.length > 0 ? (
            <ChartContainer
              config={{
                memory: {
                  label: t("dashboard.memory.label"),
                  color: "var(--color-primary)",
                },
              }}
              className="h-[200px] w-full"
...
function DashboardAlerts() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("dashboard.recentAlerts.title")}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          {recentAlerts && recentAlerts.length > 0 ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              <p>{t("dashboard.recentAlerts.empty")}</p>
            </div>
          )}
        </div>
      </CardContent>
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense fallback={
        <div className="grid gap-4 md:grid-cols-2">
          <SkeletonChart title={t("dashboard.cpu.title")} description={t("dashboard.chart.last24h")} />
          <SkeletonChart title={t("dashboard.memory.title")} description={t("dashboard.chart.last24h")} />
        </div>
      }>
        <DashboardCharts />
      </Suspense>
      ...
      <Suspense fallback={
        <SkeletonTable rows={5} columns={3} title={t("dashboard.recentAlerts.title")} />
      }>
        <DashboardAlerts />
      </Suspense>
    </div>
