  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {paginationInfo
            ? t('domains.showing', { shown: domains.length, total: paginationInfo.totalCount })
            : t('domains.loadingList')}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {/* Search and Filters */}
        <div className="mb-4 flex flex-wrap gap-2">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[180px]">
              <SelectValue placeholder={t('domains.filter.status')} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select value={sslFilter} onValueChange={setSslFilter}>
            <SelectTrigger className="w-[180px]">
              <SelectValue placeholder={t('domains.filter.ssl')} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select value={modsecFilter} onValueChange={setModsecFilter}>
            <SelectTrigger className="w-[180px]">
              <SelectValue placeholder={t('domains.filter.modsec')} />
            </SelectTrigger>
            <SelectContent>
...
function DomainActions() {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          toast.success(t('domains.toast.deleted', { name: domain.name }));
          setConfirmDialog(prev => ({ ...prev, open: false }));
        } catch (error: any) {
          toast.error(error.message || t('domains.toast.deleteFailed'));
        }
      },
    });
  };
        ...
        try {
          ...
          toast.success(
            t('domains.toast.sslToggled', {
              name: domain.name,
              state: newSSLStatus ? t('domains.state.enabled') : t('domains.state.disabled'),
            })
          );
          setConfirmDialog(prev => ({ ...prev, open: false }));
        } catch (error: any) {
  ...
  const handleToggleStatus = async ({ domain, enabled }: { domain: any; enabled: boolean }) => {
    try {
      ...
      toast.success(
        t('domains.toast.domainToggled', {
          name: domain.name,
          state: enabled ? t('domains.state.enabled') : t('domains.state.disabled'),
        })
      );
    } catch (error: any) {
      toast.error(error.message || t('domains.toast.domainToggleFailed'));
    }
  };
...
export default function Domains() {
  ...
  const handleSave = async (domainData: any) => {
    try {
      if (editingDomain) {
        ...
        toast.success(t('domains.toast.updated', { name: domainData.name }));
        // Only close dialog on success
        setDialogOpen(false);
        setEditingDomain(null);
      } else {
        await createDomain.mutateAsync(domainData);
        toast.success(t('domains.toast.created', { name: domainData.name }));
        // Only close dialog on success
        setDialogOpen(false);
        setEditingDomain(null);
      }
    } catch (error: any) {
      const errorMessage = error?.response?.data?.message || error.message || t('domains.toast.saveFailed');
  ...
  const handleReloadNginx = async () => {
    try {
      ...
      toast.success(t('domains.toast.nginxReloaded'));
    } catch (error: any) {
      toast.error(error.message || t('domains.toast.nginxReloadFailed'));
    }
  };
      ...
      <div className="space-y-6">
        ...
        <InstallationProgressDialog
          ...
          onComplete={() => {
            ...
            toast.success(t('domains.toast.installComplete'));
          }}
        />
      </div>
    </TooltipProvider>
