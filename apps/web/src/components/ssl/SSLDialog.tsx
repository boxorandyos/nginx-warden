...
export function SSLDialog({ open, onOpenChange, onSuccess }: SSLDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.domainId) {
      toast.error(t('ssl.toast.selectDomain'));
      return;
    }

    if (method === 'manual') {
      if (!formData.certificate || !formData.privateKey) {
        toast.error(t('ssl.toast.certKeyRequired'));
        return;
      }
      ...
      if (!certValidation.valid) {
        toast.error(certValidation.error || t('ssl.toast.invalidCert'));
        return;
      }
      ...
      if (!keyValidation.valid) {
        toast.error(keyValidation.error || t('ssl.toast.invalidKey'));
        return;
      }
      ...
      if (formData.chain && formData.chain.trim()) {
        ...
        if (!chainValidation.valid) {
          toast.error(chainValidation.error || t('ssl.toast.invalidChain'));
          return;
        }
      }
      ...
      if (selectedDomain) {
        ...
        if (!domainMatchValidation.valid) {
          toast.warning(domainMatchValidation.error || t('ssl.toast.domainWarning'));
    ...
    try {
      if (method === 'auto') {
        ...
        toast.success(t('ssl.toast.uploaded'));
      }
      ...
      toast.error(error.response?.data?.message || t('ssl.toast.addFailed'));
    }
  };
